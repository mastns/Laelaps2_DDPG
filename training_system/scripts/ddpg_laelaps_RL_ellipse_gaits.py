...
if __name__ == "__main__":
    test_condition=input("Specifiy the test condition for running the algorithm: ")
    ...
    save_folder=os.path.dirname(os.path.realpath(__file__))+"/runs/"+str(newDirName)+"_ddpg"+test_condition
    ...
    writer=env.tensorboardwriter(train_folder,use_tensorboardEnV=False,use_tensorboardAlg=True)
    writer_val=validation_env.tensorboardwriter(test_folder,use_tensorboardEnV=False,use_tensorboardAlg=True)
    ...
    act_opt = optim.Adam(act_net.parameters(), lr=learning_rate) # use different optimizers not to complicate how we deal with gradients
    ...
    with ptan.common.utils.RewardTracker(writer,min_ts_diff=120) as tracker: # min_ts_diff print mean reward every 120 seconds
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            while True:
                ...
                if frames % validation_steps == 0:
                    ...
                    writer_val.add_scalar("test_reward", rewards, frames) # the sum of the reward happened until termination (one episode) in every step divided by how many the model is validated
                    writer_val.add_scalar("test_steps", steps, frames) # the sum of test episodes steps divided by how many the model is validated
