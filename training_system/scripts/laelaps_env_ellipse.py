...
# -*- coding: utf-8 -*- 
'''
Written by Yehia Salah Elbahrawy in the scope of the work of Master Thesis at the University 
Duisburg-Essen. The work presented is a cooperation between University Duisburg-Essen and 
...
controllers_list = ["joint_state_controller","RF_knee","RF_hip","RH_knee","RH_hip","LF_knee","LF_hip","LH_knee","LH_hip"]
...
init_angles=[0.21644,-0.30604, 0.21644,-0.30604, 0.21644,-0.30604, 0.21644,-0.30604] # init angles are not 0 to avoid legs exploding in simulation and have more stable initial position
...
class LaelapsEnvEllipse(gym.Env):
    ...
    def __init__(self): 
        ...
        self.last_base_position = [0, 0, 0] # fixed frame is nearly on the origin of the ground 0,0,0.6 at laelaps body center of mass
        ...
        self.controllers_object = ControllersConnection(namespace="laelaps_robot", controllers_list=controllers_list)

        #_______________SUBSCRIBERS__________________________________________________ 

        rospy.Subscriber(gazebo_model_states_topic,ModelStates,self.models_callback) #give base position and quaternions 
        rospy.Subscriber(joint_state_topic, JointState, self.joints_state_callback) # give motor angle, velocity, torque 
        ...
        observation_high = (self.GetObservationUpperBound() + observation_eps) # the epsilon to avoid 0 values entering the neural network in the algorithm
        observation_low = (self.GetObservationLowerBound() - observation_eps)
    ...
    def GetObservationUpperBound(self):
        ...
        upper_bound[3:6] = [2 * math.pi / self.time_step] * 3 # Roll, pitch, yaw rate
...
# _______________call back functions____________________
    ...
    def models_callback(self,msg): 
        ...
        if self.ramp_available:
            self.ramp_inclination=msg.pose[self.ramp_model_number].orientation # both ramps have the same inclination
            
        if episode_start: 
            if self.saving_option:
                self.base_zaxis.append([self.base_pos.z,self.base_velocity.z])
                self.base_x_y.append([self.base_pos.x,self.base_pos.y])
                euler= self.transfrom_euler_from_quaternion([self.base_orientation.x,self.base_orientation.y,self.base_orientation.z,self.base_orientation.w])
                self.euler_angles.append(euler)
                self.euler_rates.append([self.base_angular_velocity.x,self.base_angular_velocity.y,self.base_angular_velocity.z])
    ...
    def transfrom_euler_from_quaternion(self,quaternion): 
        ...
        M = np.array(H, dtype=np.float64, copy=False)[:3, :3] #take only first three rows and first three coloumns = rotation matrix 
    ...
    def GetObservation(self):
        observation = []
        _,base_orientation_quaternion,_,euler_rate = self.GetLaelapsBaseInfo() # returns base: position, orientation(in quaternion), linear velocity, angular velocity
        euler=self.transfrom_euler_from_quaternion(base_orientation_quaternion)
    ...
    def publish_motor_angles(self,motor_angle):
        ...
        for index in range(8):
            ...
            x = threading.Thread(target= self.publish_threads_motor_angles, args=(index,motor_angle))
            threads.append(x)
            x.start()
    ...
    def pubilsh_toe_commands(self,toe_x_y,phase_shift):
        ...
        for index in range(len(laelaps_feet)):
            # toe_class_msg=Toe()
            x = threading.Thread(target= self.publish_threads_toe, args=(index,toe_x_y,phase_shift))
            threads.append(x)
            x.start()
    ...
    def init_reset(self): 
        ...
        self.ros_wait(0.01) # wait some time until robot stabilizes itself in the environment
        self.gazebo.pauseSim()
    ...
    def reset(self): 
        ...
        self.last_base_position=[0.0,0.0,0.0] # reset the variable base position to 0 
        self.episode_step=0
        self.episode_reward=0


        self.episode_number+=1
        episode_savingpath = str(saving_path) +"/Episode_" +str(self.episode_number)
        if self.saving_option:
    ...
    def termination(self):
        ...
        if self.ramp_available:
            ramp_pitch=self.GetRampInclination() # pitch angle of the ramp, inclined slopes goal 3.2 m 
            is_fallen = math.fabs(roll) > 0.3 or math.fabs(pitch) > 0.3+math.fabs(ramp_pitch) or math.fabs(yaw) > 0.2  or math.fabs(pos[0]) >= self.env_goal  or math.fabs(pos[1]) > 0.2   #3.2 m is nearly the end of the ramp
        else:
            ramp_pitch=0 #flat terrain goal 6 m 
            is_fallen = math.fabs(roll) > 0.3 or math.fabs(pitch) > 0.3+math.fabs(ramp_pitch) or math.fabs(yaw) > 0.2  or math.fabs(pos[0]) >= self.env_goal  or math.fabs(pos[1]) > 0.2 or math.fabs(pos[2]) > 0.17

        if is_fallen:
            self.save_time("Finish") # episode finish
            self.save_termination([math.fabs(roll),math.fabs(pitch),math.fabs(yaw) , math.fabs(pos[0]), math.fabs(pos[1]) ])
    ...
    def reward(self):
        ...
        if any(nan_check):
            rospy.loginfo("---Angles Inverse Kinematics: NaN %s, try again---r=-1",nan_check)
            reward=-1
        else:
            if self.ramp_available:
            ...
            forward_reward = math.fabs(current_base_position[0]/math.cos( ramp_pitch )) - math.fabs(self.last_base_position[0]/math.cos(ramp_pitch))
            ...
            # Penalize drifting 
            drift_reward  = math.fabs(current_base_position[1]) - math.fabs(self.last_base_position[1])
            ...
            reward = (self.distance_weight * forward_reward  - self.drift_weight * drift_reward ) 
        return reward
    ...
    def save_torques(self,torques,step_number): 
        ...
        if self.saving_option:
            ...
            np.save(str(episode_savingpath)+"/Torques_"+str(step_number),torques)
    
    def save_euler_angles(self,euler_angles,step_number): 
        global episode_savingpath
        if self.saving_option:
            euler_angles=np.array(euler_angles,dtype=np.float64)
            np.save(str(episode_savingpath)+"/EulerAngles_"+str(step_number),euler_angles)
    ...
    def save_angular_velocities(self,angular_velocities,step_number): 
        ...
        if self.saving_option:
            ...
            np.save(str(episode_savingpath)+"/AngularVelocities_"+str(step_number),angular_velocities)

    def save_baze_zaxis(self,base_zaxis,step_number): 
        global episode_savingpath
        if self.saving_option:
            base_zaxis=np.array(base_zaxis,dtype=np.float64)
            np.save(str(episode_savingpath)+"/BaseZaxis_"+str(step_number),base_zaxis)

    def save_baze_x_y(self,base_x_y,step_number): 
        global episode_savingpath
        if self.saving_option:
            base_x_y=np.array(base_x_y,dtype=np.float64)
            np.save(str(episode_savingpath)+"/BaseX_Y"+str(step_number),base_x_y)
    ...
    def save_time(self,start_or_finish):
        ...
        if self.saving_option:
            ...
            np.save(str(episode_savingpath)+"/Episode_"+str(start_or_finish),episode_time)

    def save_termination(self,termination_critiria):
        global episode_savingpath
        if self.saving_option:
            np.save(str(episode_savingpath)+"/termination",termination_critiria)
    def close(self):
