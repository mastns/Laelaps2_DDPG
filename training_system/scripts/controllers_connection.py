...
class ControllersConnection():
    ...
    def reset_controllers(self):
        ...
        print("RESET CONTROLLERS",reset_result)
        return reset_result
    ...
    def turn_off_controllers(self):
        ...
        if result_off_ok:
            rospy.loginfo("Deactivated Controlers")
        else:
            rospy.loginfo("Controllers deactivation failed!")
    def turn_on_controllers(self):
        result_on_ok = self.switch_controllers(controllers_on = [],controllers_off = self.controllers_list)
        if result_on_ok:
            rospy.loginfo("Controllers Reseted==>"+str(self.controllers_list))
        else:
            rospy.loginfo("Failed to restart controllers result_on_ok==>" + str(result_on_ok))
